...
export default function AdmissionEnquiryPage() {
    const jobDetails : Record<string, string[]> = {
      'Teaching': ['PYP Home Room Teachers', 'EYFS Home Room Teachers', 'Primary Teacher (PRT)', 'Asst. Teachers', 'Trained Graduate Teacher (TGT)'],
      'Non Teaching' : ['Design & Innovation Coach', 'Learning Experience Designer', 'Technology Integration Specialist']
      }

    const availableSubjects = ['Music', 'Art', 'Dance', 'IT']

    const boards = ['IB', 'IGCSE', 'CBSE',  'ICSE', 'Oxford', 'Other']
    ...
    const [availablePosition, setAvailablePosition] = useState<string[]>([]);


    // useEffect(() => {
    //     if (formData.academicYear && classOptions[formData.academicYear]) {
    //       setAvailablePosition(classOptions[formData.position]);
    ...
    const handleCoverLetterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {        
          ...
          if (!coverletterfile) return alert("Please select cover letter.");
  
          const documentFiles = new FormData();
          documentFiles.append("resume", resumefile);
          documentFiles.append("coverletter", coverletterfile)
          ...
          if (uploadRes.ok) {
            formData.resumeUrl = uploadData.resumeUrl;
            formData.coverletterUrl = uploadData.coverLetterUrl;
          }
          ...
          if (res.ok) {
            setMessage("Career Form submitted successfully!");
            ...
            setMessage(data.message || "Error submitting Career Form");
            setMessageType("error");
          }
        } catch (error) {
          setMessage("Error submitting career form. Please try again.");
          setMessageType("error");
        } finally {
          setLoading(false);
        }
      ...
      <div className="w-1/4 rounded-l-md bg-sky-500 text-white p-8 flex flex-col justify-between">
        <div>
          <div className="flex flex-col items-center mt-12 my-6">
            ...
            <h1 className="text-xl font-semibold text-center">CAREERS AT <br/>THE KULISH SCHOOL, JAIPUR</h1>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className='block text-sm text-gray-600 mb-1 mr-2'>Job Type <span className='text-red-600'>*</span></div>
                <div className='flex gap-5'>
                    <div className='items-center justify-center mr-2'>
                        <input 
                            ...
                            className="mr-2"
                            required />
                        <label className="text-sm text-gray-600 mb-1">Teaching</label>
                    </div>
                    <div className='items-center justify-center mr-2'>
                        <input 
                        type="radio" 
                        ...
                        required
                        />
                        <label className="text-sm text-gray-600 mb-1">Non Teaching</label>
                    </div>
                </div>  
              </div>
              <div></div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Position Applied For <span className='text-red-600'>*</span></label>
                <select 
                ...
                >
                  ...
                  {formData.jobtype!='' && jobDetails[formData.jobtype].map((className) => (
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Subject <span className='text-red-600'>*</span></label>
                <select 
                  className="border rounded p-2 w-full text-sm"
                  name="subject"
                  value={formData.subject}
                  onChange={handleChange}
                  disabled={!(formData.position=="Primary Teacher (PRT)" || formData.position=="Trained Graduate Teacher (TGT)")}
                  required
                >
                  <option value="">Select Subject</option>
                  {availableSubjects.map((className) => (
                    <option key={className} value={className}>{className}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Notice Period <span className='text-red-600'>*</span></label>
                <select 
                  className="border rounded p-2 w-full text-sm"
                  name="noticeperiod"
                ...
                >
                  ...
                  <option value="15 Days - 1 Month">15 Days - 1 Month</option>
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Current CTC <span className='text-red-600'>*</span></label>
                <input 
                  type="text" 
                  name="ctc"
                  value={formData.ctc}
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Current Employer <span className='text-red-600'>*</span></label>
                <input 
                  type="text" 
                  name="employer"
              ...
              <div>
                <div className='block text-sm text-gray-600 mb-1 mr-2'>Current Board <span className='text-red-600'>*</span></div>
                <div className='flex gap-5'>
                    { boards.map((board)=>
                    <div className='items-center justify-center mr-2'>
                        <input 
              ...
              <div>
              <label className="block text-sm text-gray-600 mb-1">(If Other Board is selected)</label>
                <input 
                  type="text" 
                  name="otherBoard"
                  ...
                  onChange={handleChange}
                  className="border rounded p-2 w-full text-sm"
                  disabled={formData.currentBoard!='Other'}
                />
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Pin Code <span className='text-red-600'>*</span></label>
                <input 
                  type="text" 
                  name="pincode"
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">City <span className='text-red-600'>*</span></label>
                <input 
                  type="text" 
                  name="city"
                  value={formData.city}
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Email Id <span className='text-red-600'>*</span></label>
                <input 
                  type="email" 
                  name="email"
          ...
          <div className="bg-white shadow-md mt-6 mb-6 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-600 mb-1">Upload Resume <span className='text-red-600'>*</span></label>
                <input 
                  type="file" 
                  name="resume"
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Upload Cover Letter <span className='text-red-600'>*</span></label>
                <input 
                  type="file" 
                  name="coverLetter"
                  accept=".pdf"
